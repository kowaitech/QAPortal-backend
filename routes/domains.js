 ...
 * /domains:
 *   post:
 *     summary: Create a new domain (staff only)
 *     tags: [Domains]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *       400:
 *         description: Bad request (name required or domain already exists)
 *       403:
...
router.post("/", auth, requireRole("staff"), async (req, res) => {
  try {
    ...
    if (!name) {
      logger.warn("Create domain failed: Name required", {
        user: req.user?._id,
      });
 ...
 * /domains:
 *   get:
 *     summary: List all domains with question counts
 *     tags: [Domains]
 *     security:
 *       - bearerAuth: []
 *     responses:
 ...
 * /domains/{id}:
 *   get:
 *     summary: Get domain details with questions
 *     tags: [Domains]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    logger.info("Get domain details", {
...
router.get("/:id/answers", auth, requireRole("staff"), async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id))
      return res.status(400).json({ message: "Invalid domain id" });
    logger.info("Fetching domain answers", {
      domainId: req.params.id,
      user: req.user?._id,
    });
...
router.get("/:id/tests", auth, requireRole("staff"), async (req, res) => {
  try {
    ...
    const tests = await Test.find({ domains: req.params.id })
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.info("Fetching completed users for domain", {
        domainId: req.params.id,
        testId,
      });
...
router.put("/:id", auth, requireRole("staff"), async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id))
      return res.status(400).json({ message: "Invalid domain id" });
    const { name } = req.body;
    logger.info("Update domain attempt", {
      domainId: req.params.id,
      user: req.user?._id,
    });
    const domain = await Domain.findById(req.params.id);
    ...
    if (!domain) {
      logger.warn("Update domain failed: Not found", {
        domainId: req.params.id,
      });
      return res.status(404).json({ message: "Domain not found" });
    }
    ...
    if (
      ...
      logger.warn("Update domain failed: Not creator", {
        domainId: req.params.id,
        user: req.user._id,
      });
      return res
        .status(403)
        .json({ message: "Can only edit domains you created" });
    }
...
router.delete("/:id", auth, requireRole("staff"), async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id))
      return res.status(400).json({ message: "Invalid domain id" });
    logger.info("Delete domain attempt", {
      domainId: req.params.id,
      user: req.user?._id,
    });
    const domain = await Domain.findById(req.params.id);
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Can only delete domains you created" });
    }
    ...
    logger.info("Domain deleted", {
      ...
      answersDeleted: aRes.deletedCount,
    });
